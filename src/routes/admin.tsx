...
import {
  FaUsers, FaVideo, FaTags, FaUsersCog, FaStore, FaImages,
  FaGift, FaCog, FaTachometerAlt, FaPlus, FaEdit, FaTrash,
  FaCheck, FaTimes, FaEye, FaEyeSlash, FaSave, FaSignOutAlt,
  FaSpinner, FaArrowLeft, FaWhatsapp, FaPhone, FaMapMarkerAlt,
  FaCoins
} from "react-icons/fa";
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <StatCard title="Total Coins" value={stats.coins} icon={FaCoins} color="gold" />
      </div>
    </div>
  );
}
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3">
                  <button onClick={() => deleteUser(u.id)} className="text-red-500 hover:text-red-700"><FaTrash /></button>
                </td>
              </tr>
            ))}
          </tbody>
            ...
            {videos.map(v => (
              <tr key={v.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3 flex gap-2">
                  <button onClick={() => { setEditing(v); setForm(v); setShowForm(true); }} className="text-blue-500"><FaEdit /></button>
                  <button onClick={() => deleteVideo(v.id)} className="text-red-500"><FaTrash /></button>
                </td>
              </tr>
            ))}
          </tbody>
...
// ========== BUSINESS (DADAZ) - FULL MANAGEMENT ==========
            ...
            {businesses.map(b => {
              ...
              return (
                <tr key={b.id} className="border-t border-gray-100">
                  ...
                  <td className="px-4 py-3">{contact?.is_confirmed ? <FaCheck className="text-green-500" /> : <FaTimes className="text-red-500" />}</td>
                  <td className="px-4 py-3 font-semibold">{b.coin_wallets?.[0]?.balance_sq || 0} SQ</td>
                  <td className="px-4 py-3 flex gap-2">
                    <button onClick={() => openEditModal(b)} className="text-blue-500 hover:text-blue-700"><FaEdit /></button>
                    {contact && <button onClick={() => deleteBusinessContact(b.id)} className="text-red-500 hover:text-red-700"><FaTrash /></button>}
                  </td>
                </tr>
              );
            })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white border shadow-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={closeEditModal} className="text-gray-500 hover:text-gray-700"><FaTimes className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleSaveBusiness} className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                <button type="submit" className="bg-purple-600 text-white px-6 py-2 rounded-lg flex items-center gap-2"><FaSave /> Save Changes</button>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setShowForm(true)} className="bg-purple-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"><FaPlus /> Create Link</button>
      </div>
      {showForm && (
        <div className="bg-white p-6 rounded-xl shadow-sm border mb-6">
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
          ...
          <tbody>
            {links.map(l => (
              <tr key={l.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3"><button onClick={() => deleteLink(l.id)} className="text-red-500"><FaTrash /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
