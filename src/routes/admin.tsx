...
import { FaUsers, FaVideo, FaTags, FaUsersCog, FaStore, FaImages, FaGift, FaCog, FaTachometerAlt, FaPlus, FaEdit, FaTrash, FaCheck, FaTimes, FaEye, FaEyeSlash, FaSave } from "react-icons/fa";
...
function AdminDashboard() {
  ...
  const tabs = [
    ...
    { id: "groups", label: "Groups", icon: FaUsersCog },
    { id: "business", label: "Business (Dadaz)", icon: FaStore },
    { id: "stories", label: "Stories", icon: FaImages },
    { id: "redeem", label: "Redeem Links", icon: FaGift },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <StatCard title="Users" value={stats.users} icon={FaUsers} color="blue" />
        <StatCard title="Videos" value={stats.videos} icon={FaVideo} color="purple" />
        <StatCard title="Groups" value={stats.groups} icon={FaUsersCog} color="green" />
        <StatCard title="Total Coins" value={stats.coins} icon={FaGift} color="gold" />
      </div>
    </div>
  );
}
...
function VideosContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    setShowForm(false);
    setEditing(null);
    setForm({ title: "", description: "", video_url: "", thumbnail: "", price_sq: 0, status: "pending", category: "" });
    fetchVideos();
  };
            ...
            {videos.map(v => (
              <tr key={v.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3 flex gap-2">
                  <button onClick={() => { setEditing(v); setForm(v); setShowForm(true); }} className="text-blue-500 hover:text-blue-700"><FaEdit /></button>
                  <button onClick={() => deleteVideo(v.id)} className="text-red-500 hover:text-red-700"><FaTrash /></button>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button onClick={addCategory} className="bg-purple-600 text-white px-4 py-2 rounded-lg">Add</button>
      </div>
          ...
          <li key={c.id} className="flex items-center justify-between px-4 py-3">
            {editing === c.id ? (
              ...
              <>
                <span>{c.name}</span>
                <div className="flex gap-2">
                  <button onClick={() => setEditing(c.id)} className="text-blue-500"><FaEdit /></button>
                  <button onClick={() => deleteCategory(c.id)} className="text-red-500"><FaTrash /></button>
                </div>
              </>
            )}
          </li>
          ...
          <tbody>
            {groups.map(g => (
              <tr key={g.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3 flex gap-2">
                  <button onClick={() => { setEditing(g); setForm(g); setShowForm(true); }} className="text-blue-500"><FaEdit /></button>
                  <button onClick={() => deleteGroup(g.id)} className="text-red-500"><FaTrash /></button>
                </td>
              </tr>
            ))}
          </tbody>
...
function BusinessContent() {
  ...
  const toggleConfirm = async (id, current) => {
    await supabase.from("business_contacts").update({ is_confirmed: !current }).eq("id", id);
    fetchContacts();
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-6">Business (Dadaz) Contacts</h2>
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left">Contact Price</th>
          ...
          <tbody>
            {contacts.map(c => (
              <tr key={c.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3">{c.services || "—"}</td>
                <td className="px-4 py-3">{c.contact_price || 0} SQ</td>
                <td className="px-4 py-3">{c.is_confirmed ? <FaCheck className="text-green-500" /> : <FaTimes className="text-red-500" />}</td>
                <td className="px-4 py-3 flex gap-2">
                  <button onClick={() => toggleConfirm(c.id, c.is_confirmed)} className="text-blue-500">
                    {c.is_confirmed ? <FaEyeSlash /> : <FaEye />}
                  </button>
                  <button onClick={() => deleteContact(c.id)} className="text-red-500"><FaTrash /></button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <tbody>
            {stories.map(s => (
              <tr key={s.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3 flex gap-2">
                  {s.status !== "approved" && <button onClick={() => updateStatus(s.id, "approved")} className="text-green-500"><FaCheck /></button>}
                  {s.status !== "rejected" && <button onClick={() => updateStatus(s.id, "rejected")} className="text-red-500"><FaTimes /></button>}
                  <button onClick={() => deleteStory(s.id)} className="text-gray-500"><FaTrash /></button>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setShowForm(true)} className="bg-purple-600 text-white px-4 py-2 rounded-lg flex items-center gap-2">
          <FaPlus /> Create Link
        </button>
      </div>
          ...
          <tbody>
            {links.map(l => (
              <tr key={l.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-3">
                  <button onClick={() => deleteLink(l.id)} className="text-red-500"><FaTrash /></button>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700">SQ to TSh Rate</label>
          <input
            type="number"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">WhatsApp Channel Link</label>
          <input
            type="text"
...
// ========== UTILITY ==========
function Loading() {
  return <div className="flex justify-center items-center h-40"><FaSpinner className="animate-spin text-2xl text-purple-600" /></div>;
}